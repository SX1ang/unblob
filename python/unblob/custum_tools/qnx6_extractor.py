...
class QNX6FS:
	...
	def isBlockEmpty(self,blocknumber,blksize,blksOffset):
		...
		Data = ''.join([chr(x) if isinstance(x, int) else x for x in Data if (x if isinstance(x, int) else ord(x)) > 0])
		return len(Data) < 1
	...
	def getSnippet(self,blocknumber,blksize,blksOffset):
		...
		Data = re.sub(rb'[^\x00-\x7F]+', b'', Data) # type: ignore
		#Data = re.sub(r'[^\x00-\x7F]+','', Data) # type: ignore
		Data = bytes([x for x in Data if x < 128])
		#Data = ''.join([x for x in Data if ord(x) < 128])
		Data = bytes([x for x in Data if x > 20])
